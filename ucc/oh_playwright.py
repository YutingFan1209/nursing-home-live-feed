...
def _search_one(page, owner_name: str) -> list[UCCFiling]:
    ...
    try:
        ...
        logger.info("OH UCC %s → %d filings", owner_name, len(results))
    except Exception as e:
        logger.error("OH search failed for %r: %s", owner_name, e)
    return results


def search_oh_batch(owner_names: list[str]) -> list[UCCFiling]:
    all_results = []
...
if __name__ == "__main__":
    ...
    results = search_oh_batch(["OHIO LIVING", "TRILOGY HEALTH SERVICES"])
    for r in results:
