...
def get_feed(
    ...
    try:
        ...
        if operator:
            filters.append("""
                (d.acquiring_entity ILIKE %s
                 OR d.seller_entity ILIKE %s
                 OR EXISTS (SELECT 1 FROM unnest(d.operator_names) op WHERE op ILIKE %s))
            """)
...
@app.get("/api/stats")
def get_stats():
    """Summary stats for the feed header."""
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    COUNT(*)                                        AS total,
                    COUNT(*) FILTER (WHERE stage = 'confirmed'
                                     OR stage = 'verified')        AS confirmed,
                    COUNT(*) FILTER (
                        WHERE created_at >= NOW() - INTERVAL '90 days'
                    )                                              AS last_90_days
                FROM deals
                WHERE stage NOT IN ('dismissed')
            """)
            row = cur.fetchone()
            cols = [d[0] for d in cur.description]
            stats = dict(zip(cols, row))

            # States covered — separate query using unnest
            cur.execute("""
                SELECT COUNT(DISTINCT s)
                FROM deals, unnest(states) AS s
                WHERE stage NOT IN ('dismissed')
            """)
