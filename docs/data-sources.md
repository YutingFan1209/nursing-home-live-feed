...
- **UCC search-name blind spot (KY, NY, OH):** `main.py`'s `ky_names`/`ny_names`/`oh_names` come from `scraper.chow.get_chow_operator_names(state)` — a static snapshot of CMS's CHOW CSV (last meaningfully updated ~Jan 2026). Inside `scraper/ucc.py`, `known_operator_names` (pulled live from the `deals` table — includes entities discovered later via RSS/Gmail/EDGAR) is only used as a fallback when the CHOW list is empty, which it never is. So every automated UCC run for these three states only ever searches the same fixed ~Jan-2026 name list, regardless of what's been discovered since. Confirmed for KY on 2026-09-21: 34 of 129 distinct KY deal entity names had never been searched by any automated run; searching them directly turned up 4 real filings never seen before (one dated 2026-03-23). NY and OH were not checked but almost certainly have the same gap (same code path). **Fix:** union the CHOW list with live `known_operator_names` instead of treating CHOW as taking exclusive precedence. See memory `ucc_chow_name_blind_spot` for the full writeup and a reusable gap-check script.
- **OH UCC filing source link doesn't resolve to a functional page for viewers:** `scripts/export_deals.py` sets OH UCC-sourced deals' `source_url` to the static portal URL `https://ucc.ohiosos.gov/search` (OH has no per-filing `detail_url` the way NY does). Reported 2026-09-21 that clicking through from the live site doesn't land on a working https page. Not yet root-caused — `curl` against that URL returns a Cloudflare-managed-challenge 403 regardless of actual site status (a known false signal, see memory `oh_ucc_ip_blocked`), so this needs an actual browser check (not curl) to diagnose whether it's a dead/redirecting URL, a client-side rendering issue on OH's Angular SPA when landed on directly vs. navigated to, or something else. **Fix:** investigate with a real browser (Claude in Chrome) once picked back up, and consider whether OH's `oh_playwright.py` scraper could capture a per-filing deep link the way NY's does, instead of only ever falling back to the generic portal homepage.
