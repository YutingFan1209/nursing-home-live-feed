...
| PA | Secured party | Automated (real Chrome over CDP, auto-launched) | Confirmed 2026-09-16: turned out not to need a human-driven browser session at all — same fix as NY (`ensure_chrome_cdp` + self-navigate). Still uses the generic national operator list rather than a PA-specific facility name list (the same gap NY had before its fix), and has no known per-filing deep link yet. |
...
| UCC-1 — PA | ✅ Working, automated (2026-09-16) | Real Chrome over CDP, auto-launched; no PA-specific search-name list or deep link yet |
